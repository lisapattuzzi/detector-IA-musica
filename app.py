...
# --- FUNCIÓN DE LIMPIEZA SIMPLIFICADA (SIN ORDEREDDICT) ---
def load_weights_safe(model, path):
    state_dict = torch.load(path, map_location='cpu')
    new_state_dict = {}
    for k, v in state_dict.items():
        name = k.replace("module.", "") # Elimina el prefijo de DataParallel si existe
...
# Intentamos cargar modelos, si fallan archivos mostramos error claro
try:
    ae, cnn = load_models()
    ...
    if u1 and u2:
        ...
        st.info("👋 ¡Bienvenido! Por favor, sube los dos archivos de audio arriba para empezar el análisis.")

except Exception as e:
    st.error(f"Error al cargar los modelos: {e}")
    st.write("Asegúrate de que 'autoencoder.pth' y 'cnn_fingerprint.pth' están en tu repositorio de GitHub.")