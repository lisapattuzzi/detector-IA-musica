...
# --- INTERFACCIA ---
st.set_page_config(page_title="IA Music Detector", page_icon="🎵", layout="wide")
st.title("🎵 IA Music Detector & Fingerprinting")
...
try:
    ...
    with col1:
        u1 = st.file_uploader("Carica Brano Originale", type=["mp3", "wav"])
    with col2:
        u2 = st.file_uploader("Carica Brano da Analizzare", type=["mp3", "wav"])

    if u1 and u2:
        if st.button("🚀 ANALIZZA SIMILITUDINE"):
            with st.spinner("Confronto impronte digitali in corso..."):
                f1, s1 = get_fingerprint(u1, ae, cnn)
                f2, s2 = get_fingerprint(u2, ae, cnn)
                sim = np.dot(f1, f2) * 100
                
                st.divider()
                st.header(f"Indice di Similitudine Matematica: {sim:.2f}%")
                
                # --- LOGICA MODIFICATA ---
                if sim > 85:
                    st.success("✅ RISULTATO: IDENTITÀ ACUSTICA (STESSA CANZONE)")
                    st.write("I due file coincidono matematicamente. È lo stesso segnale audio originale.")
                elif sim > 20:
                    st.warning("⚠️ RISULTATO: BRANO ORIGINALE / DIFFERENTE")
                    st.write("Esistono alcune correlazioni, ma i brani sono acusticamente distinti.")
                else:
                    st.error("🚨 RISULTATO: COVER IA RILEVATA")
                    st.write("La similitudine è estremamente bassa. L'IA ha ricostruito la melodia cambiando radicalmente la struttura del segnale (Spettrogramma).")

                # Visualizzazione grafica
                fig, ax = plt.subplots(1, 2, figsize=(10, 3))
                librosa.display.specshow(s1, ax=ax[0], cmap='magma')
                ax[0].set_title("Spettrogramma Originale")
                librosa.display.specshow(s2, ax=ax[1], cmap='magma')
                ax[1].set_title("Spettrogramma Sospetto")
                st.pyplot(fig)
    else:
        st.info("👋 Carica entrambi i file audio per avviare il motore di analisi.")
