...
def load_weights_safe(model, path):
    try:
        ...
        raise Exception(f"No se pudo cargar el archivo {path}. Asegúrate de que esté en tu repositorio.")
...
# --- INTERFAZ ---
st.set_page_config(page_title="Detector de Música IA", page_icon="🎵", layout="wide")
st.title("🎵 Detector de Música IA y Huellas Digitales")
...
try:
    ...
    with col1:
        u1 = st.file_uploader("Cargar Canción Original", type=["mp3", "wav"])
    with col2:
        u2 = st.file_uploader("Cargar Canción para Analizar", type=["mp3", "wav"])

    if u1 and u2:
        if st.button("🚀 ANALIZAR SIMILITUD"):
            with st.spinner("Comparando huellas digitales musicales..."):
                f1, s1 = get_fingerprint(u1, ae, cnn)
                f2, s2 = get_fingerprint(u2, ae, cnn)
                sim = np.dot(f1, f2) * 100
                
                st.divider()
                st.header(f"Índice de Similitud Matemática: {sim:.2f}%")
                
                # --- LÓGICA DE CLASIFICACIÓN ---
                if sim > 85:
                    st.success("✅ RESULTADO: IDENTIDAD ACÚSTICA (MISMA CANCIÓN)")
                    st.write("Los dos archivos coinciden matemáticamente. Se trata del mismo archivo de audio original.")
                elif sim > 20:
                    st.warning("⚠️ RESULTADO: CANCIÓN ORIGINAL / DIFERENTE")
                    st.write("Existen algunas correlaciones, pero las canciones son acústicamente distintas.")
                else:
                    st.error("🚨 RESULTADO: COVER IA DETECTADO")
                    st.write("La similitud es extremadamente baja. La IA ha reconstruido la melodía cambiando radicalmente la estructura de la señal (Espectrograma).")
                ...
                ax[1].set_title("Espectrograma Sospechoso")
                st.pyplot(fig)
    else:
        st.info("👋 Por favor, carga ambos archivos de audio para iniciar el motor de análisis.")

except Exception as e:
    st.error(f"Error al cargar los modelos: {e}")
    st.write("Asegúrate de que 'autoencoder.pth' y 'cnn_fingerprint.pth' estén en tu repositorio de GitHub.")