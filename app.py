...
from collections import OrderDict # Para limpiar los nombres de las capas
...
# --- FUNCIÓN PARA LIMPIAR PESOS (SOLUCIÓN AL ERROR) ---
def clean_state_dict(state_dict):
    new_state_dict = {}
    for k, v in state_dict.items():
        name = k.replace("module.", "") # Elimina el prefijo de DataParallel
...
# --- INTERFAZ ---
st.title("🎵 Detector IA: Versión Final")
ae, cnn = load_models()

u1 = st.file_uploader("Original", type=["mp3", "wav"])
u2 = st.file_uploader("Sospechosa", type=["mp3", "wav"])

if u1 and u2:
    if st.button("CALCULAR"):
        f1 = get_fingerprint(u1, ae, cnn)
        f2 = get_fingerprint(u2, ae, cnn)
        # Usamos producto punto ya que los vectores están normalizados (Similitud Coseno)
        sim = np.dot(f1, f2) * 100
        st.metric("Similitud", f"{sim:.2f}%")
        if sim > 75:
            st.error("ES UNA COVER IA / PLAGIO")
        else:
            st.success("ES ORIGINAL / DIFERENTE")